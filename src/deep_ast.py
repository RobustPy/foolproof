...
class DeepMixin(_Base):
    ...
    def deep_visit(self, callable: Union[FunctionType, MethodType]):
        ...
        if not start_node:
            print(f"Impossible to analyze {callable}")
            return
    ...
    def _convert_to_ast_node(
        ...
        try:
            ...
            # print(f"Unable to get source for {item.__name__}")
            return None
    ...
    def _record_node(self, item: Union[FunctionType, MethodType]):
        if isinstance(item, MethodType):
            ...
            self.parent_nodes.append(f"{item.__self__.__class__.__name__}.{item.__name__}()")
            return
    ...
    def visit(self, node: ast.AST) -> Any:
        ...
        if isinstance(node, ast.Try):
            # TODO : we should check if the exception handles all possible errors in the try block
            # but for now we will just assume it does, so no need to visit the try block
            # handlers = [h.type.id for h in node.handlers]
            return

        # TODO : handle RecursionError more explicitly
        try:
    ...
    def _process_super(self, method_name: str):
        ...
        parent_search_results = [self._module_search(class_name) for class_name in parent_class_names]

        parent_class_objs = [parent_obj for parent_obj in parent_search_results if parent_obj is not None]
